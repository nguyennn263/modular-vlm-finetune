...
def cells(seed: int, resume: bool, epochs: int, limit: int | None = None) -> list[dict]:
    ...
    if resume:
        resume_setup = [code(
            ...
            "# NOTE: 2 separate mount quirks stacked here, both confirmed empirically",
            "# (not guessed): (1) Kaggle's mount layout for a dataset varies flat",
            "# (/kaggle/input/<slug>/...) vs nested (/kaggle/input/datasets/<owner>/<slug>/...)",
            "# -- same as DATA_DIR/IMAGES_DIR above, handled with a recursive glob. (2) `kaggle",
            "# datasets create --dir-mode zip` on a dir holding ONE subdirectory (checkpoint-N/)",
            "# flattens that subdirectory's files up to the dataset root on upload/unpack --",
            "# there is no checkpoint-N/ folder once mounted, just the files directly. So we",
            "# locate the mounted dir by a marker file (trainer_state.json) instead of a",
            "# checkpoint-N name, and recover the step number from trainer_state.json itself.",
            "import shutil, os, glob, json",
            f"os.makedirs('{ft_out}', exist_ok=True)",
            f"markers = glob.glob('/kaggle/input/**/{CKPT_DS_SLUG}/trainer_state.json', recursive=True) or "
            f"glob.glob('/kaggle/input/**/{CKPT_DS_SLUG}/**/trainer_state.json', recursive=True)",
            "assert markers, 'no checkpoint found in resume dataset -- ' + repr(os.listdir('/kaggle/input'))",
            "src = os.path.dirname(markers[0])",
            "gstep = json.load(open(markers[0])).get('global_step', 'resumed')",
            "step = f'checkpoint-{gstep}'",
