...
CELLS = [
    ...
    code("import json, pathlib, pandas as pd",
         "m = pathlib.Path('checkpoints/router/metrics.json')",
         "print(json.loads(m.read_text()) if m.exists() else 'no router metrics')",
         "for s in ('train','val'):",
         "    p = pathlib.Path(f'outputs/router/prq_{s}.parquet')",
         "    print(s, pd.read_parquet(p).shape if p.exists() else 'MISSING')"),
