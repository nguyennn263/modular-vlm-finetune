...
def download_and_organize_data():
    ...
    try:
        ...
        download_path = Path(download_path)  # Convert to Path object
    ...
    # Debug: List actual structure
    data_loader_logger.info(f"Dataset structure at {download_path}:")
    for item in download_path.glob('*'):
        data_loader_logger.info(f"  - {item.name} {'(dir)' if item.is_dir() else '(file)'}")
        if item.is_dir():
            for subitem in item.glob('*'):
                data_loader_logger.info(f"    - {subitem.name}")
    ...
    data_loader_logger.info(f"Keeping downloaded folder for inspection: {download_path}")
    # Note: Folder is kept in ~/.cache/kagglehub/ for debugging
    # Uncomment below to auto-cleanup after moving files:
    # try:
