...
CELLS = [
    ...
    md("### P4 — router P(r|Q): PhoBERT + 8-way head (contribution #2) + dump prq parquets"),
    code("!python -m src.cli.train_router --split-dir data/splits --epochs 3 --predict-splits train,val"),
    code("!cat checkpoints/router/metrics.json",
         "!python -c \"import pandas,glob; [print(f, pandas.read_parquet(f).shape) for f in glob.glob('outputs/router/prq_*.parquet')]\""),
    md("### P4 — f(I,Q) visual-state features"),
    code("!python -m src.cli.build_fiq --split-dir data/splits --splits val --pca 64",
         "!python -c \"import pandas; print(pandas.read_parquet('outputs/fiq/val.parquet').shape)\""),
    md("### P1 — is n_tiles a real compute lever? (final-plan section 5.2)"),
    code("!python -m src.cli.profile --n-tiles 1 2 4 6 --samples 32",
         "!cat outputs/profile/pipeline_cost.json 2>/dev/null || echo 'no profile output'"),
