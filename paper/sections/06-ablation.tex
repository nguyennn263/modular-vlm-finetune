...
\begin{tabular}{llcl}
    ...
    RQ5 $\cdot$ representation alignment & projector logit-KD ($\alpha{=}1$) & $-8.80$ & negative (mis-weighted) \\
...
\begin{tabular}{lrrrrr}
    ...
    \textbf{Multi-Token} & \textbf{7.35\,M (0.78)} & \textbf{49.55} & \textbf{53.17} & \textbf{$+3.6$~[2.9, 4.1]} & \textbf{$92.3 \to 101.7$} \\
    Light Q-Former  & 27.6\,M (2.87) & 46.25 & 53.21 & $+7.0$~[6.1, 7.4] & $83.7 \to 103.0$ \\
    Full Q-Former   & 69.4\,M (6.91) & 47.36 & 53.21 & $+5.9$~[4.8, 6.1] & $86.9 \to 102.4$ \\
    \bottomrule
  \end{tabular}}
\end{table}
...
state?} No. We build an exhaustive oracle over the $|A|=9$
$(\text{bridge} \times \ntiles)$ action space (per-sample CIDEr $M(a;x)$, cost
$C(a) = \ntiles/6$; bridges are the tile-augmented retrains). On held-out test
($3{,}739$ samples) the oracle beats the best fixed action
\bridge{multi\_token$|$t1} by $+0.36$ CIDEr ($+40\%$), spread \emph{evenly}
across all eight reasoning categories (per-category headroom $+0.28$ to $+0.65$).

That headroom is not learnable. A policy MLP trained \emph{and} tested on the
same split reproduces the oracle ($a^\ast$-match $0.98$), but $a^\ast$ does not
transfer --- the val and test oracle-$a^\ast$ distributions disagree despite
identical stratification, because per-sample CIDEr on single 4-word answers is a
poor estimator of relative quality among near-tied actions. Three held-out policy
arms --- \bridge{ours} $= P(r{\mid}Q)+f(I,Q)$, \bridge{rt\_only} $= P(r{\mid}Q)$,
\bridge{visual\_only} $= f(I,Q)$ --- all converge onto
\bridge{fixed: multi\_token$|$t1} ($a^\ast$-match $0.45$--$0.46$, mean CIDEr
$0.90$--$0.90$, $\lambda$-independent), picking that action for
$\ge 97.9\%$ of test questions. Adding the reasoning-type posterior changes
nothing. No learned routing policy beats the fixed best-bridge / minimum-tiles
policy.
...
alignment close the gap?} No (rows in Table~\ref{tab:abl-summary}, all 3-seed).
Multi-reference answer sampling slightly hurts (\dF{}~$-1.47$).
\texttt{align-feat} --- distilling the bridge toward Vintern's \emph{own}
pre-aligned \texttt{mlp1} connector --- is an \textbf{absolute null}
(\dF{}~$-0.03$): the bridge is already at the alignment the frozen decoder can
use. \texttt{align-logit} at $\alpha=1.0$ is a known mis-weighting (the KL term
swamps the generation objective); since the correctly-weighted \texttt{feat}
variant already shows zero benefit, we did not pursue a weight sweep. Nothing on
the training or alignment axis moves F1.
...
\textbf{all five converge to F1 $52.6$--$53.2$ / CIDEr-D $100.8$--$103.0$}
(Fig.~\ref{fig:bridge-equalizing}); the lift is larger for weaker bridges
($+3.6 \to +7.8$), and every plain$\to$LoRA CI clears zero. Once the decoder has
...
indistinguishable, so we keep \textbf{r=16} and make no ``higher rank is better''
claim. Training the LoRA for 3 epochs rather than 1 adds F1 $53.17 \to 54.67$
($+1.5$) and CIDEr-D $101.7 \to 106.8$, closing the F1 gap to ViMoE from $7.5$ to
$6.0$; most of the gain lands by epoch 2. The 1-epoch numbers are the headline;
the 3-epoch result is the strongest form.

\subsection{Reading}
\label{sec:abl-summary}

Four independent axes on the vision / training side of the pipeline --- none of
which touch the decoder --- produce no lift; \texttt{feat}-alignment is an
